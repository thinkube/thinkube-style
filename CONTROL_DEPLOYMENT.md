...
```
GitHub Repository (thinkube-control)
  ↓ is a Copier template
Ansible Playbook (12_deploy_dev.yaml)
  ↓ runs Copier to transform template
/shared-code directory (instantiated template)
  ↓ 12_deploy_dev.yaml pushes to
...
- **GitHub = Copier TEMPLATE** (what you edit directly)
- **Gitea = Instantiated code** (generated by Copier)
- **YOU NEVER MANUALLY PUSH TO GITEA**
- `12_deploy_dev.yaml` does: Copier transform → git push to Gitea → triggers CI/CD

**Migration Strategy:** Work in GitHub repository, deploy with Ansible ✅
...
```bash
# 1. Verify React code builds successfully
cd /path/to/thinkube-control/frontend
npm run build

# 2. Commit changes to GitHub (template repository)
cd /path/to/thinkube-control
git add .
git commit -m "Migrate frontend from Vue to React"
git push origin main  # This pushes to GITHUB, not Gitea

# 3. Deploy via Ansible (Copier transforms template → pushes to Gitea → triggers CI/CD)
cd /tmp/thinkube-installer
./scripts/run_ansible.sh ansible/40_thinkube/core/thinkube-control/12_deploy_dev.yaml

# 4. Monitor deployment (cannot use control.cmxela.com/cicd - circular dependency)
# - ArgoCD UI: https://argocd.cmxela.com
# - Harbor: https://registry.cmxela.com
# - kubectl get pods -n thinkube-control -w
```

**What happens during deployment:**
1. `12_deploy_dev.yaml` runs Copier on the GitHub template
2. Copier generates instantiated code in `/shared-code`
3. `12_deploy_dev.yaml` pushes the instantiated code to Gitea
4. Gitea webhook triggers Argo Workflow (builds image)
5. Harbor webhook triggers ArgoCD (deploys to k8s)
...
# 2. Final commit to GitHub template
cd /path/to/thinkube-control
git add .
git commit -m "Complete React migration - frontend production ready"
git push origin main  # Pushes to GitHub template repository

# 3. Deploy via Ansible (Copier will handle Gitea)
cd /tmp/thinkube-installer
./scripts/run_ansible.sh ansible/40_thinkube/core/thinkube-control/12_deploy_dev.yaml

# 4. Monitor deployment (NOT via control.cmxela.com/cicd - circular dependency!)
# - ArgoCD UI: https://argocd.cmxela.com
# - Harbor: https://registry.cmxela.com
# - kubectl get pods -n thinkube-control -w
